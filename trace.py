...
def get_trace_model(embedding_model_path, classifier_model_path, save_model_path, device):
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    embedding_model = PDDModel(1280,15,True)
    classifier_model = Perceptron_classifier(1280,15)
...
def main():
    ...
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    get_trace_model(config['embedding_model'], config['classifier_model'], config['save_model'], device)
